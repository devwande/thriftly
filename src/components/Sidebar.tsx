...
const Sidebar = () => {
  ...
  return (
    <div className={`${isMinimized ? 'w-32' : 'w-64'} bg-black rounded-r-xl min-h-screen  flex flex-col text-gray-300 transition-width duration-300`}>
        ...
        {navItems.map((item) => (
          <Link
            ...
            className={`flex items-center gap-5 px-6 py-3 whitespace-nowrap rounded-r-lg font-bold ${
              location.pathname === item.href ? 'bg-[#F8F4F0] text-black' : 'hover:text-white'
            }`}
          >
      ...
      <button
        onClick={toggleSidebar} className="flex items-center space-x-3 px-4 py-3 text-sm whitespace-nowrap hover:text-white" >
        <img src={isMinimized ? "src/assets/icons/Maximise.svg" : "src/assets/icons/Minimise.svg"} alt="toggle icon" className='p-6 transition-all' />
